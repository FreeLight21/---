...
7. 给容器注入组件的常用注解：@Bean, @Component,@Controller, @Service @Respository
8. @Import({User.class, DBHelper.class}), 给容器导入User这个组件，组件的名字就是全类名
...
7. http请求中出现的中文乱码问题：
   ...
   * Tomcat 服务器在接收到这些字节后，并不知道它们原始采用的是什么字符编码。而Tomcat 默认的编码格式为 ISO8859-1 。所以会将这些字节按照 ISO8859-1的格式进行编码，编码后在控制台显示，所以在控制台会显示乱码。
     ...
     * 针对Post请求：request.setCharacterEncoding("utf-8");该方法时是设置请求正文中字符编码，不适合get方式，get提交的参数在请求行中

     * 针对Get请求：在 Tomcat 安装目录的 conf/server.xml 中，找到端口号为 8080 的 < 标签，在其中添加 URIEncoding=UTF 8 的设置，即可将 Tomcat 默认字符编码修改为 UTF 8 。

     * 万能解决方式，无论是Post还是Get请求，都可以解决中文乱码问题。<mark>从数据在请求中的存放形式，还是数据被Tomcat服务器的servlet接收到的存放形式，都是单个字节的形式存在。</mark>节的形式存在，而在众多字符编码格式中， ISO8859-1为单字节编码，所以，首先对单字节的数据进行编码，并将编码后的数据存放在字节数组中。然后，再将字节数组中的数据，按照指定的 UTF 8 格式进行解码，即变为了需要的 UTF 8 字符编码的数据，解决了中文乱码问题。
...
4. HTTP协议主要分为两个版本：1.0/1.1， “无状态协议”， 它无法记录客户端会话状态资源，主要是通过cookie会话跟踪技术来保存会话状态，实现多个请求间的数据传递。

5. FTP:文件传输协议，主要提供文件传输服务，基于TCP实现可靠的传输。
...
6. telnet：远程登录协议， SSH: 安全的网络传输协议,专为登录远程会话和网络服务器

   >主要区别：SSH对于传输的数据进行了加密处理，而Telnet协议是明文传输、
   >
   >共同点：都是建立在可靠的传输协议TCP之上。

7. 面试常客：TCP的三次握手和四次挥手
   ...
   * 客户端–发送带有带有 ACK 标志的数据包–三次握手–服务端
     ...
     ![TCP三次握手](https://gitee.com/light21/my-picture/raw/master/img/202203291634003.png)

   >三次握手的目的：建立可靠的通信信道，双方确认自己与对方的发送与接收是正常的。

   * 第一次握手：Client 什么都不能确认；Server 确认了对方发送正常，自己接收正常
   * 第二次握手：Client 确认了：自己发送、接收正常，对方发送、接收正常；Server 确认了：对方发送正常，自己接收正常
   * 第三次握手：Client 确认了：自己发送、接收正常，对方发送、接收正常；Server 确认了：自己发送、接收正常，对方发送、接收正常

   >三次握手就能确定双方的收发功能正常，缺一不可。

8. 第二次握手传回了ACK，为什么还要传回SYN？

   * 接收端会送发送端的ACK，是为了告诉客户端，我接受到的信息确实是你发送的信号，表明了客户端到服务端的通信时正常。
   * 回传SYN是为了建立并确认服务端到客户端的通信。

9. 为什么要四次挥手？断开一个 TCP 连接则需要“四次挥手”：

   ![TCP四次挥手](https://gitee.com/light21/my-picture/raw/master/img/202203291640884.png)
...
10. TCP和UDP协议的区别：
    ...
    * UDP一般用于即时通信，例如QQ语音。。。
      ...
      ![TCP、UDP协议的区别](https://gitee.com/light21/my-picture/raw/master/img/202203291651061.jpeg)

11. TCP协议如何保证可靠传输：

    * 应用数据被分割成TCP认为合适发送的数据块，TCP给每一个包进行编号，接收方会对数据包进行 排序
    * 校验和
    * 流量控制：
    * 拥塞控制：网络拥塞对的时候，会减少数据的发送
    * 超时重传：当TCP发送一个包的报文，启动定时器，等待目的端确认这个报文包已经收到了，如果一直没有收到一个确认，将重发这个数据段。
    * ARQ协议(自动重传请求)：为了实现可靠传输，基本原理是发送完一个分组就停止发送，等待对方确认。在收到确认之后，才会再发下一个组。
...
1. 关系型数据库，表明了数据库所存储数据之间的联系(一对一，一对多，多对多)

2. 存储引擎；5.7默认的引擎是innodb
   * 是否支持行级锁： myisam只支持表级锁，一锁就将整个表的数据锁住，并发情况下性能低下。Innodb支持表级锁和行级锁，默认是行级锁。
   * 是否支持事务： myisam不支持事务。innodb支持事务，start transaction关闭事务自动提交，commit,手动提交，rollback回滚事务。
   * 是否支持外键： myisam不支持。
   * 是否支持数据库异常崩溃之后的安全恢复。innodb支付，myisam不支持，Innodb主要是通过redo log(重做日志)来保证事务的持久性，使用undo log(回滚日志)保证事务的原子性。
   
3. InnoDB存储引擎的三种算法：
   * Record Lock：记录锁，单个行记录上锁
   * Gap Lock：间隙锁，锁定一个范围，不包括记录本身。
   * Next-key Lock: Record+Gap的组合，锁定一个范围，包含记录本身。
   
4. 事务：一组完整的业务逻辑，要么都成功，要么都失败。如果没有指明，事务一般是指数据库事务，而不是分布式事务。

5. 关系型数据库都有的特性-ACID特性：
   * A原子性，事务是最小的执行单位，原子不可再分。
   * C一致性， 执行事务前后，数据保持一致。
   * I隔离性，并发访问数据库时，一个用户的事务不被其他的事务所干扰，各并发事务间的数据库时是独立的。
   * D持久性，事务提交之后，它对数据库的数据改变是持久的。
   
6. 并发事务带来的问题：
   * 脏读：当一个事务在访问数据并对数据进行了修改，但是这种修改还没提交到数据库，此时另一个事务也在访问这个数据，然后使用了这种数据，但是这个数据是没有提交的数据，因此对于另一个事务来说读到的数据是脏数据。
   
   * 幻读：它发生在一个事务读取了几行数据，接着另一个并发事务插入了一些数据。在随后的查询中，第一个事务发现多了一些本不存在的记录，就好像出现了幻觉一样。
   
   * 不可重复读：在一个事务内多次多次读取同一数据，在这个事务还没结束的时候，另一个事务也访问了该数据。那么，在第一个事务中的两次读数据之间，由于第二个事务的修改导致了第一个事务内两次读取的数据可能不太一样。--》情况成为不可重复读。
   
   * 丢失修改--》问的较少
   
     >不可重复读和幻读的区别：
     >
     >1. 可不重复读的重点：事务在多次读取同一行数据的时候，发生其中的某些列发生了修改。
     >2. 幻读的重点：事务一次读取几行记录的时候，这些记录发生了删除或者新增
   
7. 事务的隔离级别：
   * **READ-UNCOMMITTED(读取未提交)**，最低隔离级别，允许读取未提交的数据变更，可能会导致脏读，幻读，不可重复提交
   
   * **READ-COMMITTED(读取提交)**， 可以阻止脏读，但是会导致幻读，不可重复提交
   
   * **REPEATABLE-READ(可重复读)**， 这是mysql默认的隔离级别，对同一字段的多次读取结果都是一致的。幻读可能发生。
   
   * **SERIALIAZED(可串行化)**，最高的隔离级别， 脏读，幻读，不可重复提交这些问题都不会出现
   
     <img src="https://gitee.com/light21/my-picture/raw/master/img/202204031601637.png" alt="image-20220403160133586" style="zoom: 67%;" />
   
     隔离级别越低，事务请求的锁越少，大部分数据库系统的隔离级别都是READ-COMMITTED，但是MySQL默认的隔离级别是REPETABLE-READ，但是myslq的性能都没有损失多大。在分布式事务的情况下，会用到串行化的隔离级别。
   
8. Mysql日志：重点-》事务日志：重做日志(redo_log)，回滚日志(undo_log),二进制日志(binlog)。
   
9. Mysql通过redolog重做日志来保证事务的持久性，通过undo_log回滚日志来保证事务的原子性。

10. MySQL数据库的数据备份，主从，主备，主主，都需要依靠redolog来同步数据，保证数据的一致性。

11. InnoDB存储引擎对MVCC的实现：
